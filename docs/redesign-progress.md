...
| **P5c** | Site Chrome | ✅ 完成（`17d3e9a`），最终回归见 WEB-006 |
| **P4** | 组件抽取（视觉等价） | ⬜ 待排期 |
| **P5b/d/e/f** | 字体、列表、文章与页面专属视觉 | ⬜ 待排期 |
| **P6** | 资产与图片管线 | ⬜ 待排期 |
...
7 视口 × 16 路由的浏览器门禁全绿：对比度 0 失败、横向溢出 0、键盘干净、控制台 0 错误。
交互验收 11/11（含 Escape 关闭与焦点回交）。导航 + 工具 + 页脚覆盖全部 13 个结构性路由，
旧 URL 18/18 可达，大小写负向测试 4/4。

回归中发现并修复 4 个缺陷：

1. `/search/` 入口在 P5c 重写 Header 时**丢失** → 补回 `SEARCH` 工具链接
2. `/research/` 与 `/notes/` 成为**孤儿页** → 纳入页脚二级导航
3. Pagefind「清空」按钮 `opacity:0` 却仍可聚焦 → 补 `visibility:hidden`
4. 搜索框只有 `title` 无可见标签 → 补 `<label>` 并绑定 id

另核实一项非缺陷：`/history/` 内嵌两个完整归档站点，在无头环境下 DOM 求值会超时，
但生产返回 200、页面 2 秒内 `readyState=complete` —— 属自动化工具固有限制，
门禁改为只对其做 HTTP 可达性检查并记录原因。
