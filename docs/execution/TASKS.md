...
Delivered：`studio/archive/import.mjs` + 迁移 v9 + CLI `archive` 子命令 + 21 项测试。

**先说已有的**：`archive_entries` 与 `memories` 本来就是两张表（KB-001），
`exportAll` 也已经带走全部档案。所以本任务补的是**导入流程** ——
原先只有「一条一条写」的 `importArchiveEntry`，没有面向用户手上文件的那一层。

Evidence：

- **增量、去重、幂等**：目录导入第二次跑「新增 0 · 已有 4」。
  实测：3 个文件（含子目录、含一个含两条的 JSON）→ 4 条；
  再跑一次一条不增。
- **形态不拘**：Markdown（`.md`/`.markdown`）、结构化 JSON（数组或单对象，
  认 `content`/`text`/`body` 三种字段名）、以及**不认识的扩展名按 Markdown 处理**。
  最后这条是刻意的：用户手上的导出文件常是 `.txt` 或没有扩展名，
  而「因为扩展名不认识所以没导入」是一种极难被发现的丢失。
- **跳过必被看见**：缺失内容的条目进 `errors` 并带上序号（`第 2 条没有 content…`），
  CLI 逐条打印。这一层的失败方式是静默丢失 —— 只说「跳过 3 条」
  等于让人自己去猜是哪 3 条。
- **不跟随符号链接**：一个指向父目录的链接会把整个家目录卷进档案。
  有用例守着（含 `/etc` 与文件级链接）。
- **不抓取**：没有 URL 导入、没有账户同步、没有自动发现。
  「未获确认不得抓取」的落点是**根本没有那个函数**，不是默认关闭。
- **冲突可标记**：`findConflicts()` 报出「同一来源、内容却不同」的档案
  （同一份导出文件更新后再导一次）。

#### 过程中修掉两个真问题

1. **去重只按内容哈希，跨来源会塌成一条。** 原先 UNIQUE 索引只建在 `hash` 上，
   于是同一段文字来自两份不同文档时，先导的那份赢了，**后一份的来源与时间
   无声消失**。而底档的价值有一半在来源上。迁移 v9 改判据为 `(source, hash)`：
   同一份文件重复导入仍幂等，同一段文字出现在两份文档里则各留一条。
   这是用例逼出来的 —— 我原本断言「第二个来源应多出一条」，结果是 1 不是 2。
2. **`contentHash` 不归一化行尾。** CRLF 与 LF、结尾换行是**传输产物**，
   不归一化会让重导一次就多出一条「新版本」，而 `findConflicts` 会把它报成冲突。
   **误报的冲突比不报更糟** —— 它让人去查一个不存在的问题，查几次之后
   就不再相信这个提示了。刻意**不**归一化行首空白：在 Markdown 里那是缩进代码块，
   抹掉它是篡改不是去重（有用例把这条边界钉住）。

未完成：档案的可视化浏览界面（现在只有 CLI 的 list/conflicts）。
...
Delivered：`docs/research/external-ai-access.md`（可行性核实）+
`studio/access/tokens.mjs`（受限身份）+ 迁移 v8 + CLI `token` 子命令 + 18 项测试。

#### 连接方式核实：**「仅凭 API Key 就能让 ChatGPT 访问本地知识库」是错的**

三个环节都不成立，且都是官方文档写明的：

1. **ChatGPT 不接受粘贴 API Key** 作为 MCP 连接器的认证方式 ——
   它支持 OAuth（含 DCR/CIMD）、无认证、混合模式，没有「贴 key」这一档
2. **服务必须公网 HTTPS 可达**。官方明确：私网/VPN 连不上，本地开发要开隧道
3. 需要**付费套餐 + 开发者模式**；Business/Enterprise/Edu 还要管理员先开权限

本服务只绑回环（`BIND_HOST` 是常量），因此**现状下外部 AI 无法直接访问**。
不是配置问题，是没有那条路径。

**记为后续集成任务**（不假装已具备）：MCP 服务器实现（传输 + OAuth 授权服务器 +
工具清单）、隧道的安全边界重估、连接器侧权限同步（平台行为）。
**无认证模式明确不采用** —— 对本项目的私人内容等于公开。

#### 真正的交付：受限访问身份

即使将来真开了隧道，也不该把主令牌交出去 —— 它是全权的，拿到就能读全部
私人笔记、记忆、对话历史，还能发起对话（等于把私人内容送到模型端点）。

三个预设：`searcher`（只搜）/ `reader`（只读）/ `drafter`（可提草稿）。
三条规则：**默认拒绝**（未分类路径一律要 `admin`，忘分类的后果是够不着而不是
放行）、**只存哈希**（同 ADR-026）、**审计只记谁访问了什么，不记内容**。

#### 过程中自己抓到的两个真问题

1. **`DELETE /api/notes/:id` 被算成了 `write:draft`** —— 我原先只按「GET 还是
   非 GET」二分，于是**一个「可以提草稿」的身份能删掉任何一篇笔记**。
   现在只有 `POST /api/notes` 算 write:draft，改与删都要 admin。
2. **受限身份的写入没被标记成 `ai_draft`** —— 能力检查只回答「能不能碰这个
   端点」，不回答「写出来的东西算谁的」。于是外部 AI 写的内容会以
   `origin: 'human'` 进入，与用户亲手写的无法区分。现在一律记为 ai_draft。

另外把「身份」从共享的 `deps` 对象上挪走：`deps` 是 `createServer` 建好、
所有请求共用的同一个对象，挂上去会让并发请求在 await 处互相覆盖身份 ——
这类错误不报错，只会间歇性地判错人。

**仍未做的**：MCP 服务器、OAuth、隧道路径。这三项都需要真实公网端点才能验证。
...
- **`POST /api/preview`**：用公开站**同一个**渲染管线渲染（`renderDocument`）。
  不是「尽量复用」而是唯一路径 —— `check:preview` 会逐字比对这条路径的产出
  与公开站产物，任何分叉都会被挡住。
- **`/prose.css`**：正文样式取自项目本身，不复制一份。
- 界面加了预览切换。**刻意不做并排**：`prose.css` 的标题用
  `clamp(…, 4vw, …)` 定尺寸，而 `vw` 跟的是视口宽度不是容器宽度 ——
  半宽面板里排出来的字号与真正发布的不一样，那种「预览」反而误导。
  全宽切换则与实际页面在同一视口宽度下，看到的才是真的。
- 预览请求带序号判重：请求是异步的，先发的可能后回；不判序号的话
  快速输入时会看到旧内容覆盖新内容，而且不报错。
- **编辑会话**（迁移 v7 + `studio/db/session.mjs`）：存「上次在编辑什么」，
  含**未保存的正文**。放库里而不是 localStorage —— 浏览器存储扛不住
  换浏览器、清缓存、隐私窗口，而这个需求的字面意思就是「重启后还在」。

浏览器验收（真实 Chrome）：预览渲染出标题/加粗/行内代码/KaTeX 公式、
正文让位、prose 样式已加载；**关掉服务再起来**，未保存的草稿标题与正文
一字不少地恢复，提示「已恢复上次未保存的内容」；无控制台错误。

顺带修掉一处死代码：`getSession` 里原有一个 `orphaned` 标记，想区分
「在写新笔记」与「在编的笔记被删了」。但外键的 `ON DELETE SET NULL`
已经把 `note_id` 置空，那个条件**永远不会成立**。与其留一个从不触发的
分支让后来的人以为它在工作，不如去掉 —— 两种情况对用户是一样的：
内容都在，接着写就是了。
...
Delivered：`studio/editor/patch.mjs` + 路由 `POST|PUT /api/notes/:id/patch` + 16 项测试。

**它要挡的不是「改得不好」，是「改了别处而没人发现」** ——
一次「帮我润色这句」顺手动了三段之外的内容，diff 里混在一堆改动中间，
而人只会看自己圈出来的那块。

Evidence：

- **边界权威**：选区由**编辑器**给出，不由 AI 声称。每个 hunk 必须落在选区内，
  否则**整份作废**。敏感性已验证：临时移除那一行校验，对应用例立刻失败。
- **不做部分应用**：一处越界 / 一处陈旧 → 全不作废。部分应用会产生
  「一半是 AI 改的、一半是原来的」文档，而没有任何东西标出分界在哪。
- **陈旧检测**：每个 hunk 声称的 `before` 必须与当前内容逐字相符，
  否则说明它是对旧版本生成的 —— 放它过去会覆盖掉用户刚写的东西。
- **source gap 是结构里的字段**，不是让模型自由发挥写在解释里。
  AI 被要求改进一段它其实没有依据的文字时，正确的输出是**说出哪里没有依据**，
  而不是编一个更好的版本。做成必填字段是为了让它必须被显示出来。
- **拿不到行号的块明确报错**，不静默跳过 —— 代码块经 Shiki 重排后位置丢失；
  跳过会让改动看起来「应用成功」了而实际没改。
- **可接受/拒绝是两个动作**：`POST` 只校验并返回预览，`PUT` 才落库。
  合并成一个的话，人在按下「生成」时就已经应用了，「可接受/拒绝」就不再存在。
- 校验失败返回 **422 而不是 500**：「它想改选区之外的地方」这句话本身
  就是要给人看的拒绝理由。
- **能力归属**：patch 改写已有内容，因此落在 `admin` 一侧 ——
  一个「可以提草稿」的外部身份不该能借这个接口改掉用户的正文（有用例）。

未完成：

- **patch 的生成**（让模型输出这份结构）。协议与校验已就绪，
  但还没有「把选区与上下文喂给模型、要它回一份 patch」的那一段。
- **编辑器界面**（显示 diff、接受/拒绝按钮）。路由已可用。
- **patch 的持久化留痕**：当前应用即写入，没有留下「这条改动是 AI 提议的」
  之外的记录。若将来需要追溯每一次 AI 提议，要加一张表。
...
Delivered：`studio/ai/gateway.mjs` + 17 项测试。

**先说已有的**：KB-003 的 `provider.mjs` 已经解决了超时与取消，也守着隐私边界。
缺的是三件事，本任务补的正是它们：

- **能力抽象**。本地模型与云端的差别不只是地址：有的没有嵌入端点、
  有的不支持工具调用。原先调用方只能「反正都调，不行再说」，
  于是失败以「端点返回 404」的形式出现 —— 既不好懂，也没告诉你该换哪一家。
  现在 `capabilities` **必填且必须显式列出**：默认给「什么都会」很方便，
  但那等于把「这家其实没有嵌入端点」藏到第一次调用失败为止。
  不支持的请求在发出去之前就被拒绝，并说明**这个端点会什么**。
  挑供应商也是按能力挑（`pick(null, 'embeddings')` 不会拿到那个只会聊天的）。
- **重试**。只重试**可能自己好起来**的：网络抖动、429、5xx。
  4xx（除 429）重试是纯粹的浪费 —— 请求本身有问题，再发十次也一样，
  而且会把一个立刻能报的错拖成几秒后才报。退避带抖动：固定间隔会让同时失败的
  多方在同一刻一起重试，把本该缓过来的服务再打一次。**取消不重试**（有用例）。
- **用量可见**。按供应商分别累计请求数、失败数、重试次数与 token。
  端点**不回 usage 时给 `null` 而不是编一个 0** —— 用 0 会让「这次没上报」
  和「这次真的用了 0」长得一样（B fixture 专门测这一条）。

#### fixture：两个行为不同的端点

验收要求「至少两个 OpenAI-compatible 或不同供应商 fixture」。这里刻意让它们
**行为不同**：A 是标准端点、回 usage；B 前 N 次返回 503、且**不回 usage**。
两个一模一样的假端点证明不了中立性，只能证明「同一段代码跑两遍结果一样」。

覆盖：两个 fixture 分别能聊且能分辨是谁答的、usage 有无的两种处理、
按供应商分开的用量、5xx 重试后成功、重试耗尽如实失败、4xx 不重试、
取消立刻停且不重试、超时被报成超时、非本机端点**在定义时**就拒绝。

未完成：把网关接进对话路由（`chat.mjs` 仍直接用 `provider.mjs`）。
...
**与 ADR-019 的关系**：那条否掉的是**节点-连线式关系图**（17 篇只有 7 条边，
画出来只能靠编造关系填满画布）。本条建的是它底层缺的那层**索引**，
不产出图形，只产出可导航的结构。

Delivered：`src/content.config.ts` 三个可选字段（`sources` / `prerequisites` /
`related`）+ `src/utils/knowledge.mjs` + 文章页「知识坐标」区块 +
`check:content` 的悬空引用校验。

Evidence：

- **全部来自人工声明，不做标签推断**。理由与 ADR-019 删除「同分类兜底」是同一条：
  一个靠推断填满的列表看起来与真实的一样，读的人无从分辨哪条是作者的意思。
- **反链是算出来的**：谁把「我」写进 prerequisites/related，谁就出现在我的
  「被引用」里。按标题排序而不是遍历顺序 —— 否则同样内容两次构建得到不同页面，
  diff 全是噪声（有用例守着）。
- **悬空引用是 error 不是 warning**。写错一个字母不会报错，页面上只是**少一个
  链接**，静默得没人会发现 —— 而那正是「事实来源可追溯」最容易被破坏的方式。
  敏感性已验证：临时写 `transfomer`（拼错）、自引用、不存在的文章，三项全被报出。
- **渲染实测**（临时给两篇文章加声明，验完还原，不改作者原文）：
  thinking 页正确显示前置知识 / 相关阅读 / 事实来源（含一个只有 note 没有 URL 的
  来源）；**ai 页出现了「被引用」并链回 thinking** —— 反链方向正确；
  外链带 `rel="noreferrer"`。
- **零声明时零影响**：三个字段全空时该区块完全不渲染，
  `check:coverage` 与 `check:preview` 均未变化。

未完成：**修订**（`updated` 字段已有且在用，但没有修订历史视图）与
**公开搜索的节点维度**（Pagefind 覆盖全文，尚未按节点聚合）。
这两条不阻塞后续任务，记为 P2。

#### 一个决定要留在这里

三个字段目前**全部为空** —— 17 篇文章没有任何一篇声明过它们。
这是刻意的：往别人的文章里塞 `sources:` 是替作者做判断，
而「这条结论依据什么」只有作者知道。填充是内容决策，不是工程任务。
