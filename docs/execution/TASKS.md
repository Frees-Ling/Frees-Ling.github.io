...
Delivered：`studio/media/library.mjs` + 迁移 v10（`media` / `media_derivatives` /
`media_refs`）+ 29 项测试。

**内容寻址**：id = 字节的 sha256 前 32 位。一次解决三件事 —— 稳定（改名、移动
都不影响）、去重（导两次自然只剩一条）、可校验（文件与记录对不上立刻能发现）。
落盘走「临时文件 → fsync → rename」，`rename` 在同一文件系统上是原子的，
因此进程在任意时刻被杀，库里要么没有这个文件、要么是一个**完整**的文件。

**EXIF 策略选的是「原图留、派生物不留」**，理由不是「哪个文件更干净」，
而是**哪个文件会离开这台机器**：母版只在本地，它的完整信息是资产；
派生物是要发出去的，那里不该带拍摄地点。同时记下每张图是否含 EXIF / GPS，
让用户有权知道自己手里有哪些照片带着位置信息。

**崩溃恢复区分两种不一致**，因为它们的含义完全不同：`missing`（库里有磁盘上
没有）是真的丢了，引用它的地方会变裂图；`orphan`（磁盘上有库里没有）是崩溃残留，
内容还在、可以收养。混成一个「不一致」计数会让人以为它们同样严重。
`adoptOrphan` **不自动跑**，且收养时校验内容哈希与 id 是否相符。

**本任务实测推翻的三件事**（都属于「看起来对」那一类）：

1. **`.withMetadata(false)` 会*保留* EXIF，不是剥离。** 直觉上「显式关掉元数据」
   应该这么写，实测那反而让 Copyright 出现在输出里，连 WebP 派生物也带着。
   默认管线才是剥离。这是隐私缺陷：派生物是要发出去的那个文件。现已改为
   不调用任何东西，并留下注释防止有人「好心」加回去。
2. **靠「用 `withMetadata()` 往返一次再找 `Exif\0\0` 标记」判定有无 EXIF 是错的** ——
   它会**造出**一个 EXIF 块（实测 279 字节的图往返后变 967 字节且带上了标记），
   于是给每一张图都盖章，`hasExif` 变成恒真。判据只能是 `meta.exif` 本身。
3. **GPS 不能靠搜字节判定。** 搜 `0x8825` 两个字节两个方向都会错：别的数据里
   恰好出现就误报，而大端（`MM`）相机上标签是反着存的、直接漏报 ——
   漏报意味着告诉用户「这张图不含位置」然后它被发出去。改为按 TIFF 结构解析
   IFD0 条目，并**同时查 XMP**（Lightroom 之类把经纬度写在那里而不是 EXIF）。

另外两处：`sharp` 的 `withExif({ GPS: ... })` **静默忽略 GPS 键**，
用它造「带 GPS 的测试图」得到的其实是不带 GPS 的图 —— 用例会因错误的原因通过；
测试改为手工拼接 APP1/EXIF 段（真机就是这么放的），并覆盖小端与大端两种字节序。
测试的 `withStore` 曾写成 `return fn(dir)`，`finally` 在异步回调兑现前就删掉了
临时目录，一批用例是靠 `importMedia` 顺手重建目录才「通过」的。

三个判据各自做过变异验证：还原旧逻辑后对应用例确实变红。
